...
/**
 * Handle work-dir switch command locally without AI.
 * Returns reply text if handled, or null to let AI process.
 */
function handleWorkDirCommand(text, settings) {
	// Patterns: 把工作目录切换到xxx / 切换工作目录到xxx / 工作目录改成xxx
	//           set work dir to xxx / change work directory to xxx
	const patterns = [
		/切换.*工作目录.*?[到至]\s*(.+)/i,
		/工作目录.*?[改为改成设置为切换为到至]\s*(.+)/i,
		/工作目录.*?[=：:]\s*(.+)/i,
		/[Ss]et\s+work\s+dir\s+(?:to\s+)?(.+)/i,
		/[Cc]hange\s+work(?:ing)?\s+directory\s+(?:to\s+)?(.+)/i,
	];
	...
	for (const pattern of patterns) {
		...
		// Update settings
		const gw = settings["pi-owl-gateway"] || {};
...
async function processMessage(text, fromUserId, ctxToken, baseUrl, token) {
	...
	// Check for work-dir switch command (handled locally, no AI)
	{
		const s = readSettings();
		const reply = handleWorkDirCommand(text, s);
		if (reply) {
			try {
