 ...
 *
 * Priority:
 *   1. settings.json → gateway.workDir
 *   2. Default: ~/.pi/gateway-workspace
 */
function resolveWorkDir() {
	const settings = readSettings();
	const gw = settings.gateway || {};
	const raw = (gw.workDir || "").trim();
...
async function sendWeChatMessage(token, baseUrl, toUserId, text, contextToken) {
...
// ────────────────────────────────────────────────────────────────────────────
// Process a single WeChat message → AI → reply
// ────────────────────────────────────────────────────────────────────────────
...
async function processMessage(text, fromUserId, ctxToken, baseUrl, token) {
	...
	try {
		const ok = await sendWeChatMessage(token, baseUrl, fromUserId, final, ctxToken);
		log(`📤 WeChat: ${ok ? "sent" : "send failed"} (${final.length}/${reply.length} chars)`);
	} catch (err) {
...
async function runWeChatPoll(token, accountId, baseUrl) {
	let syncBuf = "";
...
const wxToken = settings["micah-gw-wx-token"];
const wxAccountId = settings["micah-gw-wx-account-id"];
const wxBaseUrl = settings["micah-gw-wx-base-url"] || "https://ilinkai.weixin.qq.com";

if (!wxToken || !wxAccountId) {
	log("WeChat not configured. Run: pi-owl-gateway login -p weixin");
	process.exit(0);
}
...
log("Starting WeChat poll loop...");
runWeChatPoll(wxToken, wxAccountId, wxBaseUrl).catch((err) => {
