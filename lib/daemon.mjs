...
import { readFileSync, writeFileSync, existsSync, appendFileSync, mkdirSync, renameSync, unlinkSync } from "node:fs";
...
// ────────────────────────────────────────────────────────────────────────────
// In-band commands (processed without AI)
// ────────────────────────────────────────────────────────────────────────────

/**
 * Handle in-band commands locally without AI.
 * Returns reply text if handled, or null to let AI process.
 *
 * Supported commands:
 *   setup-work-dir <path>         — Set workspace directory (primary)
 *   设置工作目录 <path>            — Chinese alias
 */
function handleLocalCommand(text, settings) {
	let dir = "";

	// Prefix matching — clean, no regex ambiguity
	const lower = text.trim().toLowerCase();

	if (lower.startsWith("setup-work-dir ")) {
		dir = text.slice("setup-work-dir ".length).trim();
	} else if (lower.startsWith("设置工作目录 ")) {
		dir = text.slice("设置工作目录 ".length).trim();
	}
	...
	// Validate: must be absolute path (resolve on Windows gives drive letter)
	if (!dir.includes(":") && !dir.startsWith("/")) {
		return `❌ 无效路径: ${dir}\n请使用绝对路径，例如: setup-work-dir D:/projects/xxx`;
	}
	...
	if (!raw["pi-owl-gateway"]) raw["pi-owl-gateway"] = {};
	const platform = "weixin";
	if (!raw["pi-owl-gateway"][platform]) raw["pi-owl-gateway"][platform] = {};
	raw["pi-owl-gateway"][platform]["work-dir"] = dir;
	...
	// Dispose session so next message uses new workDir
	disposeSession();
...
async function processMessage(text, fromUserId, ctxToken, baseUrl, token) {
	...
	// Check for in-band local commands (no AI involved)
	{
		const s = readSettings();
		const reply = handleLocalCommand(text, s);
		if (reply) {
			try {
				await sendWeiXinMessage(token, baseUrl, fromUserId, reply + "\n\nEND!", ctxToken);
				log(`📤 WeiXin: sent (local command)`);
			} catch (err) {
